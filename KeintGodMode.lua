if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Keint Segs is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "KeintSegs is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Keint Segs v" .. currentVersion
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		if everyClipboard then
			toClipboard("https://t.me/keintcheck")
		...
		inviteButton.Text = "Copy Telegram Invite Link (https://t.me/keintcheck)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'telegram / support / help', DESC = 'Invite to the KeintCheck support server.'}
...
addcmd('telegram', {'support', 'help'}, function(args, speaker)
	if everyClipboard then
		toClipboard('https://t.me/keintcheck')
		notify('Telegram Invite', 'Copied to clipboard!\nt.me/keintcheck')
	else
		notify('Telegram Invite', 't.me/keintcheck')
	end
	if httprequest then
		httprequest({
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Keint Segs may have issues or features that are not functioning correctly.") end
end)
